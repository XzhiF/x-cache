...
public class PostServiceXCacheObject
{
    ...
    private static TypeReference<XCacheObject<Post>> postCacheTypeReference = new TypeReference<XCacheObject<Post>>()
    {
    };
        ...
        try {
            return localCache.get(id, () -> {
                ...
                if (StringUtils.isNotBlank(postJson)) {
                    XCacheObject<Post> postXCacheObject = JSONObject.parseObject(postJson, postCacheTypeReference);
    ...
    public Post findExpireAfter(Long id, long expireAtMillis, long redisTimoutMillis)
    {
        ...
        String postJson = redisTemplate.opsForValue().get(redisKey);
        if (StringUtils.isNotBlank(postJson)) {
            XCacheObject<Post> redisCachePost = JSONObject.parseObject(postJson, postCacheTypeReference);
     ...
     * @return
     */
    public Post findRefreshByVersion(Long id, Integer version)
    {
        // 通过本地缓存获取
        XCacheObject<Post> localCachePost = localCache.getIfPresent(id);
        if (localCachePost != null) {
            log.info("post obtain from local cache");
            return refreshByVersionIfDiff(localCachePost, id, version).getObject();
        }
        ...
        if (StringUtils.isNotBlank(postJson)) {
            ...
            localCache.put(id, redisCachePost);
            log.info("post obtain from redis");
            return refreshByVersionIfDiff(redisCachePost,id,version).getObject();
        }

        // 通过db获取
        Post post = postMapper.selectById(id);
        XCacheObject<Post> dbCachePost = XCacheObject.of(post,version);
        redisTemplate.opsForValue().set(redisKey, JSONObject.toJSONString(dbCachePost), 1, TimeUnit.DAYS);
        localCache.put(id, dbCachePost);
        log.info("post obtain from db");
    ...
    public XCacheObject<Post> refreshByVersionIfDiff(XCacheObject<Post> xCacheObject, Long id, Integer version)
    {
        if (xCacheObject.getVersion().intValue() != version.intValue()) {
            String redisKey = CACHE_POST_MODEL_KEY + id;
            Post post = postMapper.selectById(id);
            XCacheObject<Post> dbCachePost = XCacheObject.of(post, version);
            redisTemplate.opsForValue().set(redisKey, JSONObject.toJSONString(dbCachePost), 1, TimeUnit.DAYS);
